...
function assertUsableBytes(bytes) {
    if (bytes.byteLength === 0)
        throw new Error('资料文件不能为空');
    if (bytes.byteLength > MAX_SOURCE_BYTES)
        throw new Error('单个资料文件不能超过 20 MB');
}
...
async function parsePdf(bytes) {
    ...
    try {
        ...
        if (pdf.numPages > MAX_SOURCE_PAGES)
            throw new Error(`PDF 页数不能超过 ${String(MAX_SOURCE_PAGES)} 页`);
        let content = '';
        const pages = [];
...
export async function parseSkillSource(originalName, bytes) {
    assertUsableBytes(bytes);
    const extension = extname(originalName).toLowerCase();
    if (extension === '.txt' || extension === '.md') {
        const content = decodeText(bytes).trim();
        if (content.length === 0)
            throw new Error('资料文件没有可读文本');
