...
export class SkillAuthoringStore {
    projectRoot;
    constructor(projectRoot) { this.projectRoot = safeRoot(projectRoot, 'projectRoot'); }
    async save(input) {
    ...
    skillFile(draft) {
        ...
        if (draft.whenToUse !== undefined)
            lines.push(`when_to_use: ${JSON.stringify(draft.whenToUse)}`);
