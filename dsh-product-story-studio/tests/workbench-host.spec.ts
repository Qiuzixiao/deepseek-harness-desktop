...
import { resolveWorkbenchSessionRoot } from '../src/workbench-host.ts'
...
describe('dsh-workbench workspace root', () => {
  it('uses the live session cwd and never falls back to the process directory', () => {
    ...
    const sandboxPolicy = { resolve: vi.fn(() => ({ workspaceRoot: '/作品/县城往事' })) }
    ...
    expect(sandboxPolicy.resolve).toHaveBeenCalledOnce()
  })
})
