...
import { parseBilingualRecord, verifyBilingualRecords } from './bilingual-docs.mjs'
...
test('rejects malformed, incomplete, and escaping bilingual records', () => {
  assert.throws(() => parseBilingualRecord('docs/topic.i18n.yaml', `topic.md: ${'a'.repeat(40)}\n`), /exactly two/u)
  assert.throws(() => parseBilingualRecord('docs/topic.i18n.yaml', `topic.md = ${'a'.repeat(40)}\ntopic.zh.md: ${'b'.repeat(40)}\n`), /not a filename/u)
  assert.throws(() => parseBilingualRecord('docs/topic.i18n.yaml', `../topic.md: ${'a'.repeat(40)}\ntopic.zh.md: ${'b'.repeat(40)}\n`), /beside the record/u)
})
...
test('reports malformed records and missing documents in one failure', () => {
  ...
  const records = new Map([
    ...
    [resolve(root, 'docs/missing.i18n.yaml'), [`missing.md: ${'a'.repeat(40)}`, `missing.zh.md: ${'b'.repeat(40)}`, ''].join('\n')],
  ])
  assert.throws(() => verifyBilingualRecords({
    root,
