...
export function verifyMarketDependencyDirection(files) {
  ...
  if (violations.length > 0) throw new Error(`Market source must not import Desktop implementation: ${violations.join(', ')}`)
...
export function verifyTrackedMarketDependencyDirection(root) {
  const paths = execFileSync('git', ['ls-files', '-z', '--', 'dsh-community-market/src'], {
    ...
    .map(normalizePath)
  return verifyMarketDependencyDirection(paths.map(path => [path, readFileSync(resolve(root, path), 'utf8')]))
}
