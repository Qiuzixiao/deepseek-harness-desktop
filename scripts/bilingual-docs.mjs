...
export function parseBilingualRecord(recordPath, content) {
  ...
  for (const [index, line] of content.split(/\r?\n/u).entries()) {
    ...
    if (match === null) throw new Error(`${recordPath}:${String(index + 1)} is not a filename and Git blob hash`)
    ...
    if (!HASH_PATTERN.test(hash)) throw new Error(`${recordPath}:${String(index + 1)} has an invalid Git blob hash`)
    entries.push({ filename, hash })
  }
  if (entries.length !== 2) throw new Error(`${recordPath} must contain exactly two bilingual document records`)
  if (entries[0]?.filename === entries[1]?.filename) throw new Error(`${recordPath} must reference two distinct bilingual documents`)
  return entries
}
...
export function verifyBilingualRecords({ root, recordPaths, readText, hashDocument }) {
  ...
  for (const recordPath of recordPaths) {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (actual !== entry.hash) violations.push(`${normalizedRecord} is stale for ${relativeDocument}: expected ${actual}, recorded ${entry.hash}`)
      } catch (cause) {
  ...
  if (violations.length > 0) throw new Error(`bilingual documentation gate failed:\n- ${violations.join('\n- ')}`)
...
export function verifyTrackedBilingualRecords(root) {
  ...
  return verifyBilingualRecords({
    ...
    hashDocument: relativePath => runGit(root, ['hash-object', `--path=${relativePath}`, '--', relativePath]).trim(),
  })
}
