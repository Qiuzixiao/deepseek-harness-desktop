...
test('validates and freezes a reproducible plugin lock', () => {
  const bytes = archive()
  const lock = validatePluginLock({ schemaVersion: 1, dshRuntime: '0.1.1-rc.1', plugins: [expected(bytes)] })
...
test('the checked-in lock pins the three stage-zero candidates', () => {
  ...
  assert.equal(lock.dshRuntime, '0.1.1-rc.1')
...
test('audits package identity, license, bundle row, client face, and compatibility', () => {
  const bytes = archive()
  const inspected = inspectPluginArchive(bytes, expected(bytes), '0.1.1-rc.1')
...
test('rejects a modified archive and a mismatched package manifest', () => {
  const bytes = archive()
  assert.throws(() => inspectPluginArchive(Buffer.concat([bytes, Buffer.from('changed')]), expected(bytes), '0.1.1-rc.1'), /SHA-256/u)
  const wrong = archive({ version: '9.9.9' })
  assert.throws(() => inspectPluginArchive(wrong, expected(wrong), '0.1.1-rc.1'), /declares version/u)
})
