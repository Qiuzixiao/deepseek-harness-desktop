...
import { createUploadHandler, sweepExpiredUploads } from '../src/upload.ts'

function startUploadServer(): Promise<{ server: ReturnType<typeof createServer>; url: string; port: number; dir: string }> {
...
test('upload handler: DELETE rejects a sibling path sharing the storage prefix', async () => {
  const { server, url, dir } = await startUploadServer()
  const siblingDir = join(dir, 'workspace', '.dsh-uploads', 'good-session-evil')
  const siblingFile = join(siblingDir, 'important.txt')
  mkdirSync(siblingDir, { recursive: true })
  writeFileSync(siblingFile, 'keep')
  try {
    const del = await fetch(`${url}/api/upload`, {
      method: 'DELETE',
      headers: { 'x-session-id': 'good-session', 'x-file-path': siblingFile }
    })
...
test('upload sweeper scans every live session workspace', async () => {
  const first = mkdtempSync(join(tmpdir(), 'dshfu-sweep-first-'))
  const second = mkdtempSync(join(tmpdir(), 'dshfu-sweep-second-'))
  const firstUploads = join(first, '.dsh-uploads', 'first-session')
  const secondUploads = join(second, '.dsh-uploads', 'second-session')
  mkdirSync(firstUploads, { recursive: true })
  mkdirSync(secondUploads, { recursive: true })
  const old = new Date(1_000)
  utimesSync(firstUploads, old, old)
  utimesSync(secondUploads, old, old)

  await sweepExpiredUploads([() => [first, second]], 100, () => 10_000)

  assert.equal(existsSync(firstUploads), false)
  assert.equal(existsSync(secondUploads), false)
})
