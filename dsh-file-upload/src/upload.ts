...
//   - served by the Harness loopback web server
...
//   - content is sniffed at upload time and returned as a Codex-style path
//     reference; documents are read lazily through read_document.
...
export function createUploadHandler(options: UploadOptions) {
  ...
  async function handleDelete(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    const storageRoot = resolve(storage.dir)
    const target = resolve(filePath)
    const relativeTarget = relative(storageRoot, target)
    if (filePath === '' || relativeTarget === '' || relativeTarget === '..'
...
export function createSweeper(
  roots: Array<string | (() => string | undefined | readonly string[])>,
  ttlMs: number,
  intervalMs: number,
  now: () => number = Date.now
): () => void {
  ...
  const timer = setInterval(() => {
    void sweepExpiredUploads(roots, ttlMs, now).catch(err => {
...
/** Remove expired session upload directories from every currently live root. */
export async function sweepExpiredUploads(
  ...
  for (const rootEntry of roots) {
    const resolved = typeof rootEntry === 'function' ? rootEntry() : rootEntry
    ...
    for (const root of candidates) {
      ...
      for (const sessionDir of sessionDirs) {
        ...
        if (info !== null && now() - info.mtimeMs > ttlMs) {
