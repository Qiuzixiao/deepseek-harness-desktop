...
//      backend can always resolve them; uploads become path references.
...
export const Config = z.object({
  ...
  /** Allow automatic fallback to the remote OpenAI vision endpoint. */
...
export function apply(ctx: any, config: FileUploadConfig): void {
  ...
  // Image descriptions prefer an explicitly configured endpoint, then local
  // Ollama. Automatic remote OpenAI fallback requires an explicit opt-in.
  const visionKeyRef = credentialRef(config.visionApiKeyEnv)
  const resolveVisionKey = async (): Promise<string> => {
    if (config.visionEndpoint === '' && !config.allowRemoteVision) return ''
    try {
  ...
  const disposeSweeper = createSweeper(
    [
      defaultDir,
      () => ctx.sessions.list()
        .map((session: { header?: { cwd?: string } }) => session.header?.cwd)
        .filter((cwd: string | undefined): cwd is string => cwd !== undefined)
    ],
    config.uploadTtlMs,
