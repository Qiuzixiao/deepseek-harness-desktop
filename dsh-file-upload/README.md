...
> explanations discover local Ollama automatically; remote OpenAI fallback is
...
- **Image explanation for text-only models** — upload an image and the plugin can generate a **description ("讲解图片")** through an explicit `visionEndpoint` or local Ollama with a VL model. Optional OpenAI fallback requires `allowRemoteVision: true`. Multimodal routes / vision bridges keep the official `read_image` path.
...
**Vision discovery chain**: ① explicit `visionEndpoint`/`visionModel` → ② **local Ollama** at `http://localhost:11434` (picks a VL model such as DeepSeek-VL2 — images never leave the machine) → ③ OpenAI only when `allowRemoteVision` is enabled and a key is available.
...
| `visionEndpoint` | `''` | Explicit vision endpoint; empty = discover local Ollama first |
...
| `allowRemoteVision` | `false` | Allow automatic fallback to OpenAI vision when a credential is available |
...
- Uploads are served by the Harness loopback web server and isolated by live Session id.
