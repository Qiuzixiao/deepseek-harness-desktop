...
> **零配置,安装即用。** 所有功能开箱即用,带合理默认——不需要 Python、不需要下载、不需要挑选后端。图片讲解默认只自动发现本地 Ollama；远程 OpenAI 回退必须显式开启。
...
- **图片自动讲解(文本模型也能看懂)**:上传图片后可通过显式 `visionEndpoint` 或本地 Ollama(如 DeepSeek-VL2)生成图片描述("讲解图片")。OpenAI 回退需设置 `allowRemoteVision: true`；多模态模型/视觉桥接则走官方 `read_image`。
...
**视觉发现链**:① 显式 `visionEndpoint`/`visionModel` → ② **本地 Ollama**(`http://localhost:11434`,自动选择 VL 模型如 DeepSeek-VL2,图片不出本机)→ ③ 仅在开启 `allowRemoteVision` 且有凭据时使用 OpenAI 标准端点。
...
| `visionEndpoint` | `''` | 显式图片讲解端点;空 = 优先发现本地 Ollama |
| `visionModel` | `''` | 视觉模型名;空 = 自动 |
| `visionApiKeyEnv` | `OPENAI_API_KEY` | 视觉 key 的凭据引用(DSH 凭据体系) |
| `allowRemoteVision` | `false` | 有凭据时是否允许自动回退到 OpenAI 视觉端点 |
