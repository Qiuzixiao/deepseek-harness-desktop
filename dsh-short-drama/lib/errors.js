...
export function screenplayToolError(error) {
    if (!(error instanceof ScreenplayError))
        return error;
    const issues = Array.isArray(error.details.issues)
        ? error.details.issues.filter((issue) => issue !== null && typeof issue === 'object')
        : [];
    if (issues.length === 0)
        return error;
    ...
    for (const [index, issue] of issues.slice(0, 5).entries()) {
        ...
        if (issues.length > 1)
            lines.push(`issue: ${String(index + 1)}`);
        if (location !== undefined)
            lines.push(`location: ${location}`);
        if (issue.expected !== undefined)
            lines.push(`expected: ${preview(issue.expected)}`);
        if (issue.actual !== undefined)
            lines.push(`actual: ${preview(issue.actual)}`);
    ...
    if (error.details.written === false)
        lines.push('formalFilesChanged: false');
