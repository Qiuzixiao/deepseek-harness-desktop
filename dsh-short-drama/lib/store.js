...
import { LEGACY_SCREENPLAY_LAYOUT, detectScreenplayLayout, } from './layout.js';
import { MAIN_CHARACTER_FIELDS, MAIN_CHARACTER_SECTIONS, OTHER_CHARACTER_FIELDS, fieldValues, hasMainCharacterFieldTemplate, isPresentFieldValue, sectionBody, } from './character-template.js';
import { SCREENPLAY_SCHEMA_VERSION, } from './types.js';
...
function episodeScreenplayPath(layout, episode) {
    return layout.episodeScreenplayPath(episode);
}
function deliverablePath(layout, projectName) {
    return layout.deliverablePath(projectName);
}
function effectiveCharacterCount(content) {
    return Array.from(content.replace(/\s/gu, '')).length;
}
function episodeLengthRange(durationSeconds) {
...
function stableId(prefix, operationId, digest = '') {
...
function normalizeRequirements(current, changes) {
    const next = {
        ...current,
        constraints: changes.constraints?.map(value => value.trim()).filter(Boolean) ?? current.constraints,
    };
    for (const key of ['title', 'genre', 'audience', 'premise', 'endingDirection']) {
    ...
    if (changes.episodeCount !== undefined)
        next.episodeCount = changes.episodeCount;
...
function hasProjectHeading(content, projectName) {
...
function validateFlexibleArtifact(content, label, projectName) {
    if (!hasProjectHeading(content, projectName)) {
        validationFailure(`${label} must start with an H1 containing the confirmed project name`, label, [{
                field: 'title',
    ...
    if (headings.length === 0) {
        validationFailure(`${label} must contain at least one level-two facts section`, label, [{
                field: 'sections',
                expected: 'at least one ## section',
            }]);
    }
}
function validateCreativeContract(content, projectName) {
    const legacyCount = LEGACY_CONTRACT_SECTIONS.filter(section => content.includes(section)).length;
    if (legacyCount === LEGACY_CONTRACT_SECTIONS.length)
        return;
    validateFlexibleArtifact(content, 'creative-contract', projectName);
}
function validateCoreSetting(content, projectName) {
    const legacyCount = LEGACY_SETTING_SECTIONS.filter(section => content.includes(section)).length;
    if (legacyCount === LEGACY_SETTING_SECTIONS.length)
        return;
    validateFlexibleArtifact(content, 'core-setting', projectName);
}
function validateMainCharacterTemplate(content, characterName) {
    ...
    if (memoryBody.length === 0)
        issues.push({ section: '一句话记忆点', issue: 'empty' });
    ...
    if (experiences.length === 0)
        issues.push({ section: '关键经历', issue: 'requires at least one list item' });
    ...
    if (issues.length > 0) {
        validationFailure(`major character ${characterName} does not match the field-level Markdown template`, 'main-character', issues);
    }
}
function validateOtherCharacterTemplate(content) {
    ...
    const roleHeadings = [...content.matchAll(/^##\s+(.+?)\s*$/gmu)]
        ...
        .filter((heading) => heading !== undefined && heading !== '角色关系图（简要）');
    const issues = [];
    if (roleHeadings.length === 0)
        issues.push({ issue: 'at least one secondary character is required' });
...
function validationFailure(message, artifact, issues) {
...
function episodeOutlineHeadings(content) {
    const headings = [];
...
function validateCompactEpisodeOutlineBlock(content, episode) {
...
function validateEpisodeScreenplay(content, episode, durationSeconds) {
...
function validateRequirements(requirements) {
    const requiredStrings = ['title', 'genre', 'audience', 'premise', 'endingDirection'];
...
function mainCharacterPath(layout, name) {
    return layout.mainCharacterPath(name);
}
function validateFullOutline(content, projectName) {
...
function validateEpisodeOutlines(content, projectName, episodeCount) {
...
function validateEpisodeOutlineBatch(content, projectName, totalEpisodes, startEpisode, endEpisode) {
...
function outlineSegment(content, headings, index) {
...
function buildEpisodeOutlineDraftContent(draft) {
    const firstBatch = draft.batches[0];
    if (firstBatch === undefined) {
        throw new ScreenplayError('INVALID_STATE', 'episode outline draft has no batches');
    }
...
function validateContent(kind, content, characterName, projectName, episodeCount, episodeNumber, durationSeconds, allowLegacyMainCharacterTemplate = false) {
    switch (kind) {
...
function episodeNumberFromPath(layout, logicalPath) {
    if (posix.dirname(logicalPath) !== layout.screenplayDir)
        return NaN;
    return Number(posix.basename(logicalPath).match(/^episode-(\d+)\.md$/u)?.[1] ?? NaN);
}
function writingProgressAfterEpisodeEdit(layout, progress, changedPaths, changedContents = new Map()) {
...
export class ScreenplayProjectStore {
    ...
    eventsPath;
    statePath;
    constructor(workspaceRoot, layout) {
    ...
    async snapshot(view = 'summary') {
        ...
        const snapshot = { initialized: true, ...cloneState(state) };
     ...
     * lost response.
     */
    async findOperationResult(operationId, expectedType) {
        const events = await this.readEvents();
    ...
    async createProject(expectedRevision, operationId, projectName, changes, input) {
        return this.mutate(expectedRevision, operationId, 'project-created', async (current, revision, time) => {
            ...
            if (!hasProjectHeading(sources[0]?.content ?? '', normalizedName)
                ...
                throw new ScreenplayError('VALIDATION_FAILED', 'creative contract and core setting titles must contain the exact confirmed project name');
            }
            ...
            const version = { id: versionId, revision, artifacts, createdAt: time };
            const state = {
    ...
    async createOutline(expectedRevision, operationId, outlineContent) {
    ...
    async createEpisodeOutlineBatch(expectedRevision, operationId, input) {
        return this.mutate(expectedRevision, operationId, 'episode-outline-batch-created', async (current, revision, time) => {
            ...
            if (startEpisode !== expectedStart) {
                validationFailure('episode outline batch must continue from the next ungenerated episode', 'episode-outlines', [{
                        field: 'startEpisode',
                        expected: expectedStart,
                        actual: startEpisode,
                    }]);
            }
            const existingEpisodeArtifact = currentVersion.artifacts.find(artifact => artifact.logicalPath === this.layout.episodeOutlinesFile);
            ...
            if (endEpisode < startEpisode || endEpisode > totalEpisodes
                ...
                validationFailure('episode outline batch range is outside the allowed size', 'episode-outlines', [{
                        field: 'episodeRange',
                        expected: {
            ...
            const existingOutlineArtifact = currentVersion.artifacts.find(artifact => artifact.logicalPath === this.layout.outlineFile);
            ...
            validateEpisodeOutlineBatch(batchContent, previous.projectName, totalEpisodes, startEpisode, endEpisode);
            const batch = {
                startEpisode,
                endEpisode,
            ...
            if (complete) {
                ...
                episodeOutlinesContent = normalizeContent(buildEpisodeOutlines(totalEpisodes, nextDraft, input.forecastContent), 'episode outlines content');
                validateContent('episode-outlines', episodeOutlinesContent, undefined, previous.projectName, totalEpisodes);
            }
            else {
                episodeOutlinesContent = normalizeContent(buildEpisodeOutlineDraftContent(nextDraft), 'episode outline progress content');
                validateEpisodeOutlineBatch(episodeOutlinesContent, previous.projectName, totalEpisodes, 1, endEpisode);
            }
            const sources = [];
            for (const artifact of currentVersion.artifacts) {
                if (artifact.logicalPath === this.layout.outlineFile || artifact.logicalPath === this.layout.episodeOutlinesFile)
                    continue;
                sources.push({
                    kind: artifact.kind,
                    logicalPath: artifact.logicalPath,
            ...
            if (complete)
                delete state.episodeOutlineDraft;
            else
                state.episodeOutlineDraft = nextDraft;
            const createdFiles = [
    ...
    async createOutlineBundle(expectedRevision, operationId, input) {
        return this.mutate(expectedRevision, operationId, 'outline-created', async (current, revision, time) => {
            const previous = this.requireState(current);
            if (previous.pendingChange !== undefined) {
                throw new ScreenplayError('INVALID_STATE', 'save or discard the pending change first', {
            ...
            validateContent('episode-outlines', episodeOutlinesContent, undefined, previous.projectName, previous.requirements.episodeCount);
            const sources = [];
            for (const artifact of currentVersion.artifacts) {
                sources.push({
                    kind: artifact.kind,
    ...
    async finalizeOutlineBundle(expectedRevision, operationId, input) {
        return this.mutate(expectedRevision, operationId, 'outline-created', async (current, revision, time) => {
            const previous = this.requireState(current);
            if (previous.pendingChange !== undefined) {
                throw new ScreenplayError('INVALID_STATE', 'save or discard the pending change first', {
            ...
            const episodeOutlinesContent = normalizeContent(buildEpisodeOutlines(totalEpisodes, draft, input.forecastContent), 'episode outlines content');
    ...
    async writingContext() {
        ...
        const episodeOutlinesArtifact = snapshot.currentVersion.artifacts.find(artifact => artifact.logicalPath === this.layout.episodeOutlinesFile);
    ...
    async validateEpisodeContent(episode, content) {
        ...
        return validateEpisodeScreenplay(normalizeContent(content, `episode ${String(episode)} screenplay content`), episode, snapshot.requirements.episodeDurationSeconds);
    }
    async createEpisodeScreenplay(expectedRevision, operationId, input) {
        return this.mutate(expectedRevision, operationId, 'episode-created', async (current, revision, time) => {
            ...
            const nextProgress = {
                status: nextEpisode > totalEpisodes ? 'Completed' : 'Writing',
                totalEpisodes,
                nextEpisode,
    ...
    async mergeDelivery(expectedRevision, operationId) {
        return this.mutate(expectedRevision, operationId, 'delivery-merged', async (current, revision, time) => {
            ...
            const sources = [];
            for (const artifact of currentVersion.artifacts) {
                if (artifact.logicalPath === mergedPath)
                    continue;
                sources.push({
                    kind: artifact.kind,
                    logicalPath: artifact.logicalPath,
    ...
    async prepareChange(expectedRevision, operationId, requestedChanges) {
        ...
        return this.mutate(expectedRevision, operationId, 'change-prepared', async (current, revision, time) => {
            ...
            for (const requested of requestedChanges) {
                ...
                if (nextCharacterName !== undefined)
                    validateCharacterName(nextCharacterName);
                const targetPath = renameTo === undefined
                    ? requested.path
                    : mainCharacterPath(this.layout, nextCharacterName);
                ...
                const episodeNumber = artifact.kind === 'episode-screenplay'
                    ...
                    : undefined;
                validateContent(artifact.kind, content, nextCharacterName, previous.projectName, previous.requirements.episodeCount, Number.isSafeInteger(episodeNumber) ? episodeNumber : undefined, previous.requirements.episodeDurationSeconds, artifact.kind === 'main-character' && !hasMainCharacterFieldTemplate(oldText));
    ...
    async saveChange(expectedRevision, operationId, changeId) {
        return this.mutate(expectedRevision, operationId, 'change-saved', async (current, revision, time) => {
            ...
            const changedEpisodeFile = pending.changes.some(change => Number.isSafeInteger(episodeNumberFromPath(this.layout, change.logicalPath)));
            ...
            const nextEntries = [];
            for (const artifact of currentVersion.artifacts) {
                if (changedEpisodeFile && artifact.kind === 'merged-screenplay')
                    continue;
                if (earliestChangedEpisode !== undefined && artifact.kind === 'episode-screenplay') {
                    ...
                    if (Number.isSafeInteger(episodeNumber) && episodeNumber > earliestChangedEpisode)
                        continue;
                }
            ...
            const nextWritingProgress = writingProgressAfterEpisodeEdit(this.layout, previous.writingProgress, pending.changes.map(change => change.logicalPath), changedContents);
            const state = {
                ...base,
                phase: 'Ready',
                revision,
    ...
    async discardChange(expectedRevision, operationId, changeId) {
    ...
    async restoreVersion(expectedRevision, operationId, sourceVersionId) {
    ...
    async mutate(expectedRevision, operationId, type, transition) {
    ...
    requirePendingChange(state, changeId) {
    ...
    async readVersionContents(version) {
    ...
    async materialize(state, previousState) {
    ...
    async writeRelative(relativePath, content) {
