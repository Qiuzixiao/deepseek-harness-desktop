...
export class ScreenplayProjectStore {
    ...
    async diagnose() {
        ...
        if (episodes.length >= 6) {
            ...
            const average = (slice) => slice.reduce((sum, item) => sum + effectiveCharacterCount(item.content), 0) / slice.length;
        ...
        for (const artifact of snapshot.currentVersion.artifacts) {
            if (artifact.kind !== 'main-character' && artifact.kind !== 'other-characters')
                continue;
    ...
    async createProject(expectedRevision, operationId, projectName, changes, input) {
        return this.mutate(expectedRevision, operationId, 'project-created', async (current, revision, time) => {
            ...
            for (const source of sources)
                validateContent(source.kind, source.content, source.characterName);
            if (sources[0]?.content.split('\n', 1)[0]?.trim() !== `# 《${normalizedName}》短剧风格与创作规则`
                || sources[1]?.content.split('\n', 1)[0]?.trim() !== `# 《${normalizedName}》核心设定`) {
                throw new ScreenplayError('VALIDATION_FAILED', 'creative contract and core setting titles must use the exact confirmed project name');
            }
