...
export function screenplayToolDefinitions(ctx) {
    return [
        ...
        defineTool({
            ...
            output: { schema: { type: 'json' }, render: (_args, value) => text(value) },
            execute: async (args, exec) => {
                const outcome = await ctx.screenplayProjects.createEpisodeScreenplay(projectWorkspace(ctx, exec), args.expectedRevision, args.operationId, {
                    episodeContent: args.episodeContent,
