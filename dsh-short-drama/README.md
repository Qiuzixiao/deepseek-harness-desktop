...
Agent 初始化时自动读取并加载内置 `creative-project-organization` Skill 的完整正文。技能按内容用途、现有结构和创作者的命名语言规划文件位置，不固定目录名称、层级或根目录白名单；不要求用户编写项目配置。根目录可放符合项目规划的文件，普通续写不会自动迁移旧文件。用户明确的路径和组织方式优先。

技能源文件位于 `managed-presets/screenplay-v1/skills/creative-project-organization/SKILL.md`；桌面预设携带同内容副本供技能目录发现，测试保证两份正文一致。运行提示词只保留调用说明，具体归档判断由 Skill 维护。
