...
创作文件必须归入子目录，根目录只允许新建作为项目导航的 `README.md`（系统元数据仍由应用管理）。默认分类为 `规则/`、`设定/`、`大纲/`、`正文/`、`资料/`、`修改记录/`，分集集纲默认放入 `大纲/分集集纲/`。只在首次写文件时创建所需目录；已有合理分类则沿用，Skill 可以细化分类和命名。

短剧 Agent 会拦截新建根目录文件和将文件移动到根目录，并提示选择分类后重试。旧根目录文件仍可读写，不会自动迁移；目录本身可以正常重命名。`规则/` 存放写作规范、格式要求和创作约束。
