...
export function installScreenplayProjectScopeGuard(ctx: Context): void {
  ctx.on('tools/pre-execute', async (exec: ToolExecution, next): Promise<PreToolDecision> => {
    if (!isProjectFileTool(exec.name)) return next()
...
function currentProject(exec: ToolRunContext): { projectRoot: string; session: NonNullable<ToolRunContext['agent']>['session'] } {
  const session = exec.agent?.session
  const projectRoot = session?.header.cwd
...
function installProjectMutationTools(ctx: Context): void {
