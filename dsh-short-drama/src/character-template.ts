...
export const OTHER_CHARACTER_FIELDS = ['身份', '性格', '记忆点', '作用'] as const
...
export type CharacterTemplateSection = keyof typeof MAIN_CHARACTER_FIELDS
...
export function fieldValues(body: string): Map<string, string[]> {
...
export function isPresentFieldValue(value: string): boolean {
  return value.trim().length > 0
}

export function hasMainCharacterFieldTemplate(content: string): boolean {
  if (MAIN_CHARACTER_SECTIONS.some(section => !content.includes(section))) return false
  for (const [section, fields] of Object.entries(MAIN_CHARACTER_FIELDS) as Array<[keyof typeof MAIN_CHARACTER_FIELDS, readonly string[]]>) {
    ...
    if (fields.some(field => !values.has(field))) return false
  }
  return true
}
 ...
 */
export function hasMainCharacterFieldSyntax(content: string): boolean {
