...
function assertUsableBytes(bytes: Uint8Array): void {
  if (bytes.byteLength === 0) throw new Error('参考文件不能为空')
  if (bytes.byteLength > MAX_REFERENCE_BYTES) throw new Error('单个参考文件不能超过 20 MB')
}

function structureOf(content: string): ReferenceDocumentStructure {
...
async function parsePdf(bytes: Uint8Array): Promise<ParsedReferenceDocument> {
  ...
  try {
    ...
    if (pdf.numPages > MAX_REFERENCE_PAGES) throw new Error(`PDF 页数不能超过 ${String(MAX_REFERENCE_PAGES)} 页`)
    let content = ''
    const pages: NonNullable<ReferenceDocumentStructure['pages']> = []
...
export async function parseReferenceDocument(originalName: string, bytes: Uint8Array): Promise<ParsedReferenceDocument> {
  assertUsableBytes(bytes)
  const extension = extname(originalName).toLowerCase()
  if (extension === '.txt' || extension === '.md') {
    const content = decodeText(bytes).trim()
    if (content.length === 0) throw new Error('参考文件没有可读文本')
