...
import type {
  ...
  ReferenceRecord,
  ReferenceScope,
  ReferenceSelectionRecord,
  ReferenceUploadSelection,
  ReferenceUploadFile,
...
const PURPOSES = new Set<ReferenceUploadSelection['purpose']>([
  'story-facts',
  'plot-structure',
...
function safeOpaqueId(value: string, label: string): string {
  if (!OPAQUE_ID_PATTERN.test(value)) throw new Error(`${label}无效`)
  return value
}
...
function decodeBase64(value: string, name: string): Buffer {
  ...
  if (bytes.length === 0 || bytes.length > 20 * 1024 * 1024) throw new Error(`${name} 的内容编码无效`)
  return bytes
}

function validateSelection(selection: unknown): asserts selection is ReferenceUploadSelection {
  if (selection === null || typeof selection !== 'object' || Array.isArray(selection)) throw new Error('参考文件选区无效')
  const value = selection as Record<string, unknown>
  if (typeof value.purpose !== 'string' || !PURPOSES.has(value.purpose as ReferenceUploadSelection['purpose'])) {
  ...
  if (scope === null || typeof scope !== 'object' || Array.isArray(scope)) throw new Error('参考文件范围无效')
  const range = scope as Record<string, unknown>
  if (range.kind === 'full') return
  if (range.kind === 'pages') {
    if (!Array.isArray(range.pages) || range.pages.length === 0 || range.pages.length > MAX_REFERENCE_PAGES
      || !range.pages.every(page => Number.isInteger(page) && (page as number) > 0 && (page as number) <= MAX_REFERENCE_PAGES)
      || new Set(range.pages).size !== range.pages.length) throw new Error('参考文件页码范围无效')
    return
  }
  if (range.kind === 'heading') {
    if (typeof range.heading !== 'string' || range.heading.trim().length === 0 || range.heading.length > 500) throw new Error('参考文件标题范围无效')
    return
  }
  if (range.kind === 'paragraphs') {
    if (!Number.isInteger(range.start) || !Number.isInteger(range.end)
      || (range.start as number) < 1 || (range.end as number) < (range.start as number)
      || (range.end as number) - (range.start as number) >= 10_000) throw new Error('参考文件段落范围无效')
    return
  }
  if (range.kind === 'selected-text') {
    if (typeof range.text !== 'string' || range.text.trim().length === 0 || range.text.length > 1_000_000) throw new Error('参考文件选中文本无效')
    return
  }
...
function paragraphSelection(document: ParsedReferenceDocument, start: number, end: number): string {
  const spans = document.structure.paragraphs.filter(item => item.index >= start && item.index <= end)
  if (spans.length === 0) throw new Error('指定段落不存在')
  return spans.map(span => document.content.slice(span.start, span.end)).join('\n\n')
}

function selectedContent(document: ParsedReferenceDocument, scope: ReferenceScope): string {
  if (scope.kind === 'full') return document.content
  if (scope.kind === 'pages') {
    if (document.structure.pages === undefined) throw new Error('只有 PDF 可按页码选择')
    const pages = new Set(scope.pages)
    const spans = document.structure.pages.filter(page => pages.has(page.page))
    if (spans.length !== pages.size || spans.length === 0) throw new Error('指定页码不存在')
    return spans.map(span => document.content.slice(span.start, span.end)).join('\n\n')
  }
  if (scope.kind === 'heading') {
    const heading = document.structure.headings?.find(item => item.title === scope.heading)
    if (heading === undefined) throw new Error('指定标题或章节不存在')
  ...
  if (scope.kind === 'paragraphs') return paragraphSelection(document, scope.start, scope.end)
  const needle = scope.text.trim()
  if (needle.length === 0 || !document.content.includes(needle)) throw new Error('选中文本不在文件内')
  return needle
}
...
export class ScreenplayReferenceStore {
  ...
  constructor(readonly projectRoot: string, referenceDir: string) {
  ...
  async conflicts(names: readonly string[]): Promise<string[]> {
    const conflicts: string[] = []
    for (const raw of names) {
      const name = safeOriginalName(raw)
      if (await exists(join(this.visibleRoot, name))) conflicts.push(name)
    }
    return conflicts
  }

  async list(): Promise<ReferenceRecord[]> {
  ...
  contextSummary(): string {
    ...
    try {
      manifest = JSON.parse(readFileSync(this.manifestPath, 'utf8')) as ReferenceManifest
    } catch {
      return ''
    }
    if (manifest.version !== 1 || manifest.references.length === 0) return ''
    let selections: ReferenceSelectionRecord[] = []
    try {
      selections = readFileSync(join(this.internalRoot, 'selections.jsonl'), 'utf8')
        .split('\n')
        .filter(Boolean)
        .map(line => JSON.parse(line) as ReferenceSelectionRecord)
    } catch {
  ...
  async structure(referenceId: string): Promise<unknown> {
  ...
  async preview(originalName: string): Promise<ReferencePreview> {
  ...
  async readSelection(selectionId: string): Promise<ReferenceSelectionRecord> {
    ...
    for (const line of lines.split('\n')) {
      ...
      const record = JSON.parse(line) as ReferenceSelectionRecord
      if (record.selectionId === safeId) return record
    }
    throw new Error('参考范围不存在')
  }

  async saveBatch(files: readonly ReferenceUploadFile[]): Promise<ReferenceUploadResult> {
  ...
  private async saveBatchLocked(files: readonly ReferenceUploadFile[]): Promise<ReferenceUploadResult> {
    if (files.length === 0) throw new Error('请先选择参考文件')
    const duplicate = files.map(file => safeOriginalName(file.originalName)).find((name, index, all) => all.indexOf(name) !== index)
    if (duplicate !== undefined) throw new Error(`本次选择包含同名文件：${duplicate}`)

    const prepared = await Promise.all(files.map(async file => {
      const originalName = safeOriginalName(file.originalName)
      if (typeof file.bytesBase64 !== 'string') throw new Error(`${originalName} 的内容编码无效`)
      ...
      if (content.length === 0) throw new Error(`${originalName} 的选定范围没有文本`)
      const existsAlready = await exists(join(this.visibleRoot, originalName))
      if (existsAlready && file.replaceExisting !== true) throw new Error(`同名文件已存在：${originalName}`)
    ...
    const selections: ReferenceSelectionRecord[] = []
    const committed: Array<{
      originalName: string
      referenceId: string
      visibleBackup: boolean
      parsedBackup: boolean
      visibleInstalled: boolean
      parsedInstalled: boolean
    }> = []
    try {
      for (const item of prepared) {
        ...
        const selection: ReferenceSelectionRecord = {
      ...
      const replacedReferenceIds = new Set(records.map(record => manifest.references.find(previous => previous.originalName === record.originalName)?.referenceId).filter((value): value is string => value !== undefined))
      const nextManifest: ReferenceManifest = {
        version: 1,
      ...
      if (oldManifestText === undefined) await rm(this.manifestPath, { force: true })
      else await writeAtomic(this.manifestPath, oldManifestText)
      const selectionsPath = join(this.internalRoot, 'selections.jsonl')
      if (oldSelectionsText === undefined) await unlink(selectionsPath).catch(() => undefined)
      else await writeAtomic(selectionsPath, oldSelectionsText)
      throw error
    } finally {
  ...
  private async manifest(): Promise<ReferenceManifest> {
    await mkdir(this.internalRoot, { recursive: true, mode: 0o700 })
    try {
      const value = JSON.parse(await readFile(this.manifestPath, 'utf8')) as ReferenceManifest
