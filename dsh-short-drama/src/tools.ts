...
import type { CreateSkillDraftInput, InstallSkillInput } from './skill-authoring.js'
...
export function screenplayToolDefinitions(ctx: Context): ToolDefinition[] {
  const definitions: ToolDefinition[] = [
    ...
    defineTool({
      ...
      output: { schema: { type: 'json' }, render: (_args, value) => text(value) },
      isConcurrencySafe: () => true,
      execute: async (args, exec) => toJson(await ctx.screenplayProjects.inspectSkillForSession(
        session(exec), undefined, args.name as string | undefined,
      )),
    ...
    defineTool({
      ...
      execute: async (args, exec) => {
        if (!Array.isArray(args.entries) && typeof args.instructions !== 'string') throw new ScreenplayError('INVALID_INPUT', '安装 Skill 需要 instructions 或资料分类 entries')
        return toJson(await ctx.screenplayProjects.installSkillForSession(session(exec), {
          ...
          ...(args.whenToUse === undefined ? {} : { whenToUse: args.whenToUse as string }),
          ...(args.applicableTo === undefined ? {} : { applicableTo: args.applicableTo as string[] }),
          ...(args.entries === undefined ? {} : { entries: args.entries as InstallSkillInput['entries'] }),
          ...(args.instructions === undefined ? {} : { instructions: args.instructions as string }),
          ...(args.sources === undefined ? {} : { sources: args.sources as InstallSkillInput['sources'] }),
          ...(args.resources === undefined ? {} : { resources: args.resources as CreateSkillDraftInput['resources'] }),
          ...(args.uncertainty === undefined ? {} : { uncertainty: args.uncertainty as string[] }),
        } as InstallSkillInput))
      },
    ...
    defineTool({
      ...
      output: { schema: { type: 'json' }, render: (_args, value) => text(value) },
      execute: async (args, exec) => toJson(await ctx.screenplayProjects.writeSceneForSession(
        session(exec), args.episode as number, args.sceneNo as number, args.content as string,
      )),
      ...presentation('写入场景'),
    }),
    defineTool({
    ...
    defineTool({
      ...
      output: { schema: { type: 'json' }, render: (_args, value) => text(value) },
      isConcurrencySafe: () => true,
      execute: async (args, exec) => toJson(await ctx.screenplayProjects.inspectSkillSourceForSession(session(exec), args.path as string)),
    ...
    defineTool({
      ...
      output: { schema: { type: 'json' }, render: (_args, value) => text(value) },
      isConcurrencySafe: () => true,
      execute: async (args, exec) => toJson(await ctx.screenplayProjects.readSkillSourceForSession(
        session(exec), args.path as string, args.offset as number | undefined, args.limit as number | undefined,
      )),
    ...
    defineTool({
      ...
      output: { schema: { type: 'json' }, render: (_args, value) => text(value) },
      execute: async (args, exec) => toJson(await ctx.screenplayProjects.commitEpisodeForSession(
        session(exec), args.expectedRevision as number, args.operationId as string, args.episode as number,
        args.continuity as CreateEpisodeScreenplayInput['continuity'],
      )),
      ...presentation('提交正式剧集'),
    }),
    defineTool({
      ...
      execute: async (args, exec) => {
        ...
        return toJson(await ctx.screenplayProjects.readSkillReferenceForSession(
          current.session, args.skill as string, args.path as string, scope,
        ))
      },
