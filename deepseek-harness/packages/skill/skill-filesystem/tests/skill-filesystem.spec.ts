...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.zenwit/skills'), 'same', 'user dsh skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.zenwit/skills'), 'same', 'project dsh skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.zenwit/skills/.system'), 'hidden-system', 'hidden system')
    ...
    await writeSkill(join(noGit, '.zenwit/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.zenwit/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.zenwit/skills'), 'runtime-name', 'User loses')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.zenwit/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.zenwit/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.zenwit/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.zenwit/skills/broken-link'))
    await symlink('/dev/null', join(home, '.zenwit/skills/device-link'))
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.zenwit/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.zenwit/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    ...
    try {
      process.env.DSH_HOME = join(envHome, '.zenwit')
      ...
      await writeSkill(join(envHome, '.zenwit/skills'), 'env-skill', 'Env skill')
