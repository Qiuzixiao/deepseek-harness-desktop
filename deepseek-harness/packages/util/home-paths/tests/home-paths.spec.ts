...
describe('dsh path helpers', () => {
  it('owns the shared default DSH home directory name', () => {
    expect(DSH_HOME_DIR_NAME).toBe('.zenwit')
    expect(DEFAULT_DSH_HOME_DISPLAY).toBe('~/.zenwit')
    expect(defaultDshHome()).toBe(join(homedir(), '.zenwit'))
  })

  it('expands tilde paths without changing non-tilde paths', () => {
    expect(expandHomePath('~')).toBe(homedir())
    expect(expandHomePath('~/.zenwit')).toBe(join(homedir(), '.zenwit'))
    expect(expandHomePath('~\\.zenwit')).toBe(join(homedir(), '.zenwit'))
    expect(expandHomePath('/tmp/.zenwit')).toBe('/tmp/.zenwit')
    expect(expandHomePath('~other/.zenwit')).toBe('~other/.zenwit')
  })
  ...
  it('labels a resolved home by whether it is the default root', () => {
    expect(dshHomeDisplay(resolve(defaultDshHome()))).toBe('~/.zenwit')
