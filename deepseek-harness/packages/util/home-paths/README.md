...
Shared filesystem path helpers for Zenwit Harness user data.

## Zenwit home

`resolveDshHome()` resolves the single-root DeepSeek Harness home. Precedence, highest first: an explicit configured path, `$DSH_HOME`, then `~/.zenwit`. The harness keeps all user data under one root.
...
`dshHomeDisplay()` names an active root symbolically for user-facing paths: `~/.zenwit` for the default home, `$DSH_HOME` for any configured home. It never leaks an absolute machine path.

`DSH_HOME_DIR_NAME` owns the default user-data directory name: `.zenwit`.

`defaultDshHome()` returns the default Zenwit Harness home by joining the operating-system home directory with `.zenwit`, using Node's platform path rules.
