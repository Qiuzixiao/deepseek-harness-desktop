...
describe('AppFrame', () => {
  ...
  it('renders the details column with an empty owner share (sessionId is framework-standard)', () => {
    const { slotCalls, getByTestId, queryByTestId } = mountFrame()
    // The center conversation column is owned by the Zenwit product frame.
    expect(queryByTestId('center-content')).toBeNull()
    expect(getByTestId('details-content')).toBeTruthy()
    const keys = slotCalls.map(c => c.key)
    expect(keys).not.toContain('conversation')
    expect(keys).toContain('details')
    expect(slotCalls.find(c => c.key === 'details')!.props).toEqual({})
  })

  it('renders the details occupant before baselines settle (no loading gate)', () => {
    // No loading gate: a bare loading status reads worse than the shell's own
    // pending rendering — the details occupant mounts from first paint.
