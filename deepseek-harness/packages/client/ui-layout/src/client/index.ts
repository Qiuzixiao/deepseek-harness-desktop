 ...
 * frame-owned child slots — sidebar, details, shell.overlay (declaration =
 * exclusive render authority) — seats the layout store (panel geometry), and
 * wires the panel-action service face. The center `conversation` slot is NOT
 * declared here: the Zenwit product frame shadows AppFrame and re-declares it,
 * so the official conversation UI renders inside that frame's right pane.
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    // there); sidebar/details/shell.overlay are the frame's children, declared
    // by the same register() call that contributes AppFrame. The 'conversation'
    // entry below stays in the TYPE registry for cross-package imports
    // (ui-conversation and friends), but its runtime declaration moved to the
    // Zenwit product frame, which shadows AppFrame. Session owners never pass
     ...
     * conversation in its right pane. ui-layout keeps the TYPE here so
     * ui-conversation and the brand/input registrants import one contract.
     */
 ...
 * shell.overlay), the layout store seat, and the inject hook that hands the
