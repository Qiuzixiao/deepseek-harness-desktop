...
export const DESKTOP_BOOT_RECOVERY_SCRIPT = `(() => {
  ...
  const text = {
    ...
    rollbackConfirm: '将保存诊断并恢复最近一次成功启动的 Profile 和配置，然后重新启动 Zenwit。 / Diagnostics will be saved before restoring the last successful Profile and configuration, then Zenwit will restart.',
    working: '正在处理，请稍候… / Working…',
    restarting: '操作已接受，Zenwit 正在重新启动… / Accepted. Zenwit is restarting…',
    diagnosticsDone: '诊断导出流程已打开。 / The diagnostic export flow opened.',
    creatorOpened: '新建配置窗口已打开。 / The Profile creator opened.',
    rollbackUnavailable: '没有可用的安全回滚，请切换配置或打开终端。 / A safe rollback is unavailable. Switch Profile or open Terminal.',
  ...
  const attach = () => {
    ...
    if (!report) return;
    const panel = element('section', { 'aria-label': 'Zenwit recovery', dataset: { dshDesktopRecovery: '' } });
