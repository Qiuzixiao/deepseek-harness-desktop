/** Zenwit executable: minimal Electron bootstrap around the Host Cordis root. */
...
async function showInstallRollbackNotice(
  ...
  const copy = locale === 'zh'
    ? {
        title: '插件安装已回滚',
        message: `Zenwit 已恢复安装 ${transaction.packageName} 前的配置。`,
        detail: '上一次启动未能通过健康验证。Zenwit 已在本地保存诊断信息，并恢复 package.json、pnpm-lock.yaml 和 pnpm-workspace.yaml；诊断信息不会自动上传。',
        confirm: '知道了',
      }
    : {
        title: 'Plugin installation rolled back',
        message: `Zenwit restored the configuration from before ${transaction.packageName} was installed.`,
        detail: 'The previous startup did not pass its health check. Zenwit saved diagnostics locally and restored package.json, pnpm-lock.yaml, and pnpm-workspace.yaml. Diagnostics are not uploaded automatically.',
        confirm: 'OK',
      }
  try {
...
async function showProfileCheckpointRestoreNotice(
  ...
  const copy = locale === 'zh'
    ? {
        title: '已恢复最近一次可用配置',
        message: `Zenwit 已恢复最近一次成功启动的配置「${profileName}」。`,
        detail: '诊断信息已尽可能保存在本地；如果恢复涉及依赖声明，插件依赖也已按锁文件重新同步。Zenwit 现在将重新启动。',
        confirm: '重新启动',
      }
    : {
        title: 'Last healthy configuration restored',
        message: `Zenwit restored Profile “${profileName}” from the last successful startup.`,
        detail: 'Diagnostics were saved locally when possible. When dependency declarations were restored, plugin dependencies were synchronized from the lockfile. Zenwit will now restart.',
        confirm: 'Restart',
      }
  try {
    await dialog.showMessageBox({
...
async function start(): Promise<void> {
  ...
  try {
    ...
    if (process.platform === 'win32') app.setAppUserModelId('com.anywherelabs.zenwit')
    ...
    const ctx = await boot(
      ...
      async (hostCtx) => {
        ...
        hostCtx.provide('desktopSettingsController', new DesktopSettingsController({
          ...
          openTerminal: () => { runtime.openTerminal() },
          openTerminalAt: (path: string) => { runtime.openTerminalAt?.(path) },
          revealInFileManager: (path: string) => { runtime.revealInFileManager?.(path) },
