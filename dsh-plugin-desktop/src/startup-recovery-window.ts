...
export function renderDesktopStartupRecoveryHtml(model: DesktopStartupRecoveryViewModel): string {
  ...
  const terminalAction = model.terminalAvailable ? button('Open Terminal', 'open-terminal') : ''
  ...
  private async handleAction(action: { readonly action: string; readonly id?: string; readonly name?: string }): Promise<void> {
    ...
    try {
      if (action.action === 'home') {
        ...
        if (this.options.openTerminal === undefined) throw new Error('Terminal is unavailable for this startup stage.')
        ...
        await this.runBusy(async () => {
          ...
          this.notice = {
            ...
            body: this.options.locale === 'zh'
              ? '配置选择已保存。请重新启动 Zenwit。'
              : 'Profile selection saved. Restart Zenwit to apply it.',
          }
          this.restartReady = true
          this.refreshProfiles()
        })
        ...
        await this.runBusy(async () => {
          ...
          this.notice = this.options.locale === 'zh'
            ? { tone: 'success', title: '配置已恢复', body: '已恢复上次成功启动的配置及其快照。请重新启动 Zenwit。' }
            : { tone: 'success', title: 'Profile restored', body: 'The last successful Profile and configuration were restored. Restart Zenwit to continue.' }
          this.restartReady = true
        })
