...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `Zenwit-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'Zenwit.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing Zenwit.exe: ${portablePath}`)
  }
