...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `QNovel-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'QNovel.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing QNovel.exe: ${portablePath}`)
  }
