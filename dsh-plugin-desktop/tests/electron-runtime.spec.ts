  ...
  return {
    ...
    Menu: {
      buildFromTemplate: vi.fn((template: unknown[]) => {
        ...
        if (first?.label === 'Zenwit' && Array.isArray(first.submenu)) {
...
describe('Electron desktop runtime', () => {
  beforeEach(() => {
    ...
    updater.filename.mockImplementation((platform: string, version: string) => (
      `Zenwit-${version}-${platform === 'darwin' ? 'mac.dmg' : 'windows.exe'}`
    ))
  ...
  it('uses the native macOS frame, Dock icon, and template tray image', async () => {
    ...
    expect(electron.browserWindows[0]?.accessibleTitle).toBe('Zenwit')
    ...
    expect(electron.menuTemplates[0]).not.toEqual(expect.arrayContaining([
      expect.objectContaining({ label: 'Switch to Extended Window' }),
    ]))
  ...
  it('does not expose shell mode switching in the tray', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('darwin')
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    ...
    expect(labels).not.toContain('Switch to Extended Window')
    expect(labels).not.toContain('Switch to Enhanced Mode')
  ...
  it('uses Electron networking and confirmation-gated macOS update handoff', async () => {
    ...
    updater.download.mockResolvedValueOnce('/tmp/Zenwit-2.1.0-mac.dmg')
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})
    const release = runtime.schedule(spec)
    await runtime.mountScheduled()
    ...
    expect(updater.download).toHaveBeenCalledWith({
      platform: 'darwin',
      version: '2.1.0',
      destinationPath: '/tmp/Downloads/Zenwit-2.1.0-mac.dmg',
      request: expect.any(Function),
      signal: controller.signal,
    })
    expect(electron.shell.openPath).toHaveBeenCalledWith('/tmp/Zenwit-2.1.0-mac.dmg')
    expect(updater.record).toHaveBeenCalledWith('/tmp/dsh-desktop-user-data', {
      platform: 'darwin',
      version: '2.1.0',
      path: '/tmp/Zenwit-2.1.0-mac.dmg',
    })
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(
      activeWindow,
      expect.objectContaining({
  ...
  it('starts the downloaded Windows installer before requesting orderly exit', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('win32')
    updater.download.mockResolvedValueOnce('C:\\Updates\\Zenwit-2.1.0-windows.exe')
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: 'C:\\Updates\\Zenwit-2.1.0-windows.exe',
    })
    ...
    expect(childProcess.spawn).toHaveBeenCalledWith(
      'C:\\Updates\\Zenwit-2.1.0-windows.exe',
      ['--updated', '--force-run'],
      {
        detached: true,
        stdio: 'ignore',
    ...
    expect(updater.record).toHaveBeenCalledWith('/tmp/dsh-desktop-user-data', {
      platform: 'win32',
      version: '2.1.0',
      path: 'C:\\Updates\\Zenwit-2.1.0-windows.exe',
    })
  ...
  it.each([
    ...
    const artifact = {
      ...
      version: '2.0.1',
      path: 'C:\\Updates\\Zenwit-2.0.1-windows.exe',
    }
  ...
  it('rejects a macOS handoff when the operating system cannot open the DMG', async () => {
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: '/tmp/Zenwit-2.1.0-mac.dmg',
    })
  ...
  it('uses advanced macOS material options without exposing mode switching', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('darwin')
  ...
  it('uses native acrylic for an extended Windows 10 window', async () => {
    ...
    expect(electron.menuTemplates[0]).not.toEqual(expect.arrayContaining([
      expect.objectContaining({ label: 'Switch to Advanced Mode' }),
    ]))
