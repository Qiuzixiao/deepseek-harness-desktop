...
`dsh-plugin-desktop` runs DSH in Electron while remaining part of the ordinary Cordis composition. The installed application is named **Zenwit**. The package provides the `dsh-plugin-desktop` executable and the `dsh-desktop` alias; the registered npm package name is the reliable `npx` entry.
...
Extended mode preserves the official upstream `ui-layout`, sidebar, conversation, and details occupants. Desktop adds a fixed 52 CSS-pixel command bar above that intact frame and shifts the entire official surface below it, so upstream content never needs element-specific title-bar safe-area compensation. The command bar and official left sidebar share one material fill, forming an inverted-L surface. The main conversation surface sits inside that L and clips through a 14-pixel rounded inner corner.

The command bar remains visible and draggable while upstream overlays are open. macOS traffic lights and Windows caption controls keep their native hit regions; the Desktop terminal and restart buttons, plus contributions registered in the additive `desktop.titlebar.action` slot, explicitly opt out of dragging and remain clickable.
...
`yarn package:dir` creates an unpacked directory for the current host platform. The packaged-runtime gate rejects an application archive that omits the desktop update and terminal modules, the DSH CLI bootstrap, the bundled pnpm entry, or the physical deployment package. Electron Builder emits the root manifest, desktop runtime, and complete dependency tree under `app.asar.unpacked`; both Host profile boot and the CLI bootstrap use this physical tree so DSH profile-fallback symlinks never target a virtual ASAR directory. `build/app-icon.png` contains the QNovel application artwork and remains the Windows/Linux application icon. The build runs `scripts/generate-mac-app-icon.mjs` to center that artwork at 824 by 824 pixels on a transparent 1024 by 1024 canvas; macOS packaging and the live Dock both use the generated `build/app-icon-mac.png`. `build/tray-icon.svg` is the simplified brand-blue QNovel feather used for the system tray: the build derives a macOS template image that the system colors automatically and fixed brand-blue Windows and Linux tray images.
...
The output is `dsh-plugin-desktop\\dist\\Zenwit-2.0.4-x64-Portable.zip`. Extract it to any writable directory and launch `Zenwit.exe` without an installer, administrator access, Start Menu registration, or uninstall step. The application still keeps its profiles, logs, and caches in the normal Windows user-data directory, so this is portable distribution rather than a self-contained data sandbox. Portable archives are not handed to the NSIS updater and must be replaced manually when a new version is released. Local builds are unsigned and may trigger an Unknown publisher or SmartScreen warning; signed portable artifacts remain a release gate.

### macOS DMG smoke

`yarn dist:mac-smoke` builds one ad-hoc signed universal DMG on a native macOS host. The same package runs natively on Intel and Apple Silicon Macs. The command refuses non-macOS hosts and runs the complete product gate before packaging: repository layout and community-contract checks, the Market build and check, then the Desktop build, every TypeScript compiler face, the full unit-test suite, runtime-closure verification, CLI/Loader/profile headless smokes, and the license audit. This includes the real login-shell tests for each supported shell installed on the macOS runner. It then packages with a complete ad-hoc signature, mounts the DMG, and verifies the property list, executable bit, both `x86_64` and `arm64` slices, and `app.asar`. It mirrors `dist:win`'s secret discipline by using the explicit ad-hoc identity `-`, disabling hardened runtime and notarization, and never publishing. The artifact has no Developer ID signature or notarization ticket, so it is intended for trusted internal use after allowing applications from Anywhere; the signed and notarized universal release remains `yarn dist:mac` on a credentialed macOS machine and writes its artifact to `dsh-plugin-desktop/dist/mac-release/`.
...
- Adding or removing a profile bundle requires restarting Zenwit; the launcher does not watch profile manifests. Selecting another profile from the tray performs that restart automatically.
