...
Release operators must publish both platform artifacts under a `v<version>` GitHub Release tag before making a version discoverable. The release workflow creates the Release and uploads the Windows NSIS installer and universal macOS DMG; the application reads the latest release tag and downloads the matching Release assets.
...
- This project pins both the published DSH `0.1.0-rc.8` family and the corresponding official `deepseek-harness/` release source. Product builds still resolve published package interfaces rather than linking the source checkout.
