...
Status: delivered and built into Zenwit, including the Host/Client market, limited npm installation, receipt-backed uninstall, and direct-bundle enable/disable
...
- Protect each Market install, and each `dsh plugin add` launched through Desktop's built-in Terminal, with a configuration-level snapshot that must pass next-start health verification.
...
- The Host/Client package is delivered as a built-in Zenwit implementation.
