export const zh = {
  ...
  manualInstallBody: '以下命令由 Zenwit 根据插件信息生成，可能与仓库中的说明不同。请先检查源码，再复制到 DSH 终端执行。',
  installCommand: '安装命令',
  ...
  terminalError: '无法打开终端，请从应用菜单手动打开后再复制命令。',
  restartError: '无法请求重启 Zenwit，请稍后手动重启。',
  installableBody: '这里显示从当前来源完整目录中初步筛选出的可安装插件。选择插件后，Zenwit 仍会再次验证版本、来源和兼容性。',
  ...
  desktopRequiredTitle: '需要 Zenwit',
  desktopUnavailable: '安装、卸载、禁用和启用只在 Zenwit 中可用；你仍然可以浏览插件目录。',
  ...
  confirmInstallBody: '请确认 Zenwit 验证的 npm 包、版本和目标配置。',
  confirmUninstallTitle: '确认卸载插件',
  confirmUninstallBody: '只会卸载当前配置中由插件市场安装并验证过的这个插件。',
  confirmDisableTitle: '确认禁用外部插件',
  confirmDisableBody: 'Zenwit 将停止在当前配置中加载这个插件，但不会卸载对应的 npm 包。',
  confirmEnableTitle: '确认启用插件',
  confirmEnableBody: 'Zenwit 将恢复在当前配置中加载这个插件。对应的代码会在重启后运行。',
  confirmInstall: '确认安装',
  ...
  restartAfterOperation: '操作完成后需要重启 Zenwit，改动才会生效。',
  disableWarning: '禁用不会卸载 npm 包，也不会隔离插件代码。',
  disableRecoveryWarning: '如果插件导致 Zenwit 无法进入此页面，需要按照恢复文档手动处理。',
  ...
  restartRequiredTitle: '需要重启 Zenwit',
  restartRequiredBody: '请重启 Zenwit，让当前配置加载最新的插件状态。',
  restartLater: '稍后重启',
  restartNow: '立即重启',
  restarting: '正在重启...',
  done: '完成',
...
export const en: Record<MarketLocaleKey, string> = {
  ...
  manualInstallBody: 'Zenwit generated this command from the plugin information, so it may differ from the repository instructions. Review the source first, then copy it into Terminal.',
  ...
  terminalError: 'Terminal could not be opened. Open it from the application menu, then copy the command manually.',
  restartError: 'Zenwit could not be restarted. Restart it manually when convenient.',
  installableBody: 'This view shows plugins pre-screened from the current source’s complete catalog. Zenwit verifies the version, source, and compatibility again after selection.',
  ...
  desktopRequiredTitle: 'Zenwit is required',
  desktopUnavailable: 'Install, uninstall, disable, and enable are available only in Zenwit. You can still browse the catalog.',
  ...
  confirmInstallBody: 'Review the npm package, version, and target profile verified by Zenwit.',
  ...
  confirmDisableBody: 'Zenwit will stop loading this plugin in the active profile without uninstalling its npm package.',
  confirmEnableTitle: 'Confirm plugin enable',
  confirmEnableBody: 'Zenwit will load this plugin in the active profile again. Its code will run after restart.',
  ...
  restartAfterOperation: 'Restart Zenwit after this operation for the change to take effect.',
  disableWarning: 'Disabling does not uninstall the npm package or isolate the plugin’s code.',
  disableRecoveryWarning: 'If the plugin prevents Zenwit from reaching this page, use the manual recovery steps.',
  ...
  restartRequiredTitle: 'Restart Zenwit',
  restartRequiredBody: 'Restart Zenwit so the current profile loads its updated plugin state.',
