...
- Agent 的创作结果必须落到项目工作区，能够查看、修改、比较、撤销和导出；
- DSH 和 Desktop 上游升级后，产品能力能够通过 Profile 和插件版本边界重新验证，而不是维护一份大规模 fork。

首个版本面向内部编导，不包含商业化基础设施。成功标准是让编剧稳定完成“需求整理 -> 设定与人物 -> 大纲 -> 分集/分章 -> 正文 -> 审校 -> 导出”的真实工作，而不是先做完整 SaaS 管理后台。
...
- 提供模板化 DOCX 导出，而普通 Office 工具无法满足版式要求；
- 提供插件无法可靠实现的跨文件一致性索引。
...
第二阶段再决定是否开发。工作台只呈现作品、当前阶段、风险、待确认决策、最近产物和下一步，不重新做通用文件树、Markdown 编辑器、Git 面板或聊天界面。

优先注册标准 Client Slot；若 `dsh-better-sidebar` 通过生产审计，可同时适配其公开 `betterSidebar` service。标准 Slot 必须是可用 fallback，不能把产品绑定到一个社区 UI 插件。

## 4. 关键用户流程

### 4.1 新项目

1. 编剧添加或选择工作区，新建 Story Studio 会话；
...
交付产品 Profile、一个 Preset、六个 Skills、项目模板、插件锁和自动化场景测试。用户通过官方聊天、工作区、deliverables 和选定文件插件完成全部流程。

### 阶段 2：领域稳定化

根据内部使用记录实现确定性项目合同模块、作品状态和项目首页；接入模板化导出。没有重复出现的痛点不进入自研 UI。
