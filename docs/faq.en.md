...
Zenwit is an open-source DeepSeek Harness desktop client for Windows and macOS. It packages the official Harness local Web UI, Host service, and plugin system into a native desktop application with a window, system tray, terminal, updates, and profile management.

## Is this an official DeepSeek product?

No. Zenwit is an independent, community-maintained open-source project. It is not affiliated with or endorsed by DeepSeek. The name only describes its technical relationship with the official [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness).
...
## Does Zenwit modify official Harness?

No. The repository pins an unmodified official Harness checkout. Compatibility mode runs the upstream default Web client. Extended mode adds its command bar through an official overlay slot while preserving that layout; advanced mode adds the Desktop-owned layout. Both are implemented through Desktop plugins without editing upstream source.
...
Yes. Zenwit uses the official Harness plugin system. Open Terminal from the tray and run `dsh plugin add`, `dsh plugin remove`, or `dsh plugin update`. These commands default to the active profile, and Desktop must be restarted after plugin changes.
