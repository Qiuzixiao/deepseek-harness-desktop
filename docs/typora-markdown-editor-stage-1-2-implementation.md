...
可视化编辑接入 Milkdown 原生历史插件，工具栏提供撤销和重做，并支持平台快捷键。每份打开的文档保留编辑器实例，切换标签不会清空历史、光标和滚动位置；保存不会清空历史。草稿在文档事务完成时同步更新，修改状态由草稿与上次保存内容比较得出。撤销回打开时的文档会恢复原始 Markdown 字节，避免序列化差异产生虚假的修改标记。

关闭文档、退出工作台或切换源码/可视化模式仍会结束当前编辑历史；历史不跨重启持久化。实现决策见 [编辑历史 Agent Note](../.agents/notes/implemented/architecture/2026-09-09-visual-editor-history.md)。
