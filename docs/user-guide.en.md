...
Download the macOS or Windows installer from the product download page. Zenwit includes Electron, Node, and its pinned DSH dependencies, so normal users do not need to install Node.js or pnpm separately.
...
Plugins are extensions that add capabilities to DSH, such as models, tools, interfaces, and workflows. Zenwit uses the same plugin system as official Harness, so official plugins install and work directly; multiple plugins follow the same conventions and can be installed and used together.
...
In the terminal opened from the Zenwit tray, bare `dsh` and plugin commands without `--profile` default to the active profile:
...
An explicit `--profile <name>` always wins. Restart Zenwit after plugin changes so the new bundle enters the Loader composition.

## Opening the terminal

Choose **Open Terminal** from the tray, Desktop settings, or the extended-window command bar. The settings action has a **Restart Desktop** button beside it. macOS opens Terminal; Windows prefers Windows Terminal and falls back to PowerShell or Command Prompt when it is unavailable.
...
- **The application crashes repeatedly before the tray appears**: run the installed executable directly with the recovery option. The default Windows installation command is below; replace the path if you selected another installation directory.
  ...
  ```powershell
  & "$env:LOCALAPPDATA\Programs\Zenwit\Zenwit.exe" --export-diagnostics
  ```
